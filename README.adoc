...
To create the resource group in an efficient way, check out the [IBM Liberty on AKS]https://aka.ms/liberty-aks tool and click on the `Create` button to start the process. 
...
Crete and environment variable in your shell for the resource group names for the cluster and the database:
...
For pane `Load balancing` and `Operator and application`, you can keep the default option in this guide. For more information about the configuration in these two panes, please visit the documentation of [Create a Liberty on AKS deployment using the portal]https://learn.microsoft.com/en-us/azure/aks/howto-deploy-java-liberty-app?tabs=in-bash#create-a-liberty-on-aks-deployment-using-the-portal
...
When deployment process complete, check the created resource group and locate the container registry created during the deployment. The name of the Container Registry will be used in `[registry-name]`.
...
To locate the created Kubernetes cluster, go to the resource group overview page in the Azure Portal. Look for resources of type `Kubernetes service` and find the cluster with a name in the format `clusterXXXXXX`. Use the name of the cluster as the value replacing the `[cluster-name]`.
