...
This guide will introduce **IBM WebSphere Liberty and Open Liberty on AKS** plan, which solution automates the majority of standard steps involved in provisioning WebSphere Liberty or Open Liberty on an AKS cluster. After the initial provisioning process is complete, you are free to customize deployments further.
...
On the **Basics** pane. Under the `Resource group`, click on the `Create new` link and type in `guideGroup` as the name of the resource group.
...
After setting up the resource group, you can leave the remaining configuration at its default settings by skipping the `AKS`, `Load balancing`, and `Operator and application` panes. The default configuration will automatically set up an https://azure.microsoft.com/en-ca/products/container-registry[Azure Container Registry] in the same location where your services are deployed and link it to the resource group, and an https://learn.microsoft.com/en-us/azure/aks/tutorial-kubernetes-deploy-cluster?tabs=azure-cli[AKS cluster] will also be created. For more details on configuring these panes, please refer to the documentation of https://learn.microsoft.com/en-us/azure/aks/howto-deploy-java-liberty-app?tabs=in-bash#create-a-liberty-on-aks-deployment-using-the-portal[Create a Liberty on AKS deployment using the portal].

Select **Review + create** pane to validate your setup. After you complete the review and the validation process, click on the **Create** button to initiate the deployment process, which may take up to 20 minutes to complete.
...
Upon completion of the deployment process, navigate to the resource group overview page in the Azure Portal. Review the created resource group and search for resources of the type `Container registry`. Locate the container registry that was created during the deployment. The container registry name will be used to substitute the `[registry-name]` value of the commands in below. For example, use `acrc75e71` as seen under the `Name` column in the output.

image::Locate_ACR.png[Locate ACR,align="center"]

Alternatively, if you prefer to use command line to retrieve information about the created container registry, execute the following command. 
...
To push Docker images to your registry, run the following Azure CLI command to log in your Azure Container Registry. Replace the `[registry-name]` with the name of the container registry.
...
After you log in, you will see the following message:
...
First, get your registry server name that looks like `[registry-name].azurecr.io` or run the following command.
...
To obtain the information about the Kubernetes cluster created within the `guideGroup` resource group, you can either view the resource group overview page or execute the following command.  
