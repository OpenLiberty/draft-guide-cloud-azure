...
To locate the created Kubernetes cluster within the `guideGroup` resource group, execute the following command.  
...
-------------  ----------  ---------------  -------------------  --------------------------  -------------------  -----------------------------------------------
clusterXXXXXX  eastus      guideGroup       1.29                 1.29.7                      Succeeded            clusterXXXXXX-dns-pazznf8n.hcp.eastus.azmk8s.io
----
...
The default Kubernetes configuration file that is updated with your cluster credentials is located within the `~/.kube/config` filepath. Replacing `[cluster-name]` with the name of the cluster.
