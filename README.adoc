...
Azure offers a managed {kube} service called Azure Kubernetes Service (AKS). Using AKS simplifies the process of running Kubernetes on Azure without needing to install or maintain your own Kubernetes control plane. It provides a hosted {kube} cluster that you can deploy your microservices to. You will use AKS with an Azure Container Registry (ACR). ACR is a private registry that is used to store and distribute your container images. Note, because AKS is not free, a small cost may associated with running this guide. See the official https://azure.microsoft.com/en-us/pricing/details/kubernetes-service/[AKS pricing^] documentation for more details.

This guide will introduce IBM WebSphere Liberty and Open Liberty on AKS, which solution automates the majority of standard steps involved in provisioning WebSphere Liberty or Open Liberty on an AKS cluster. Once the initial provisioning process is complete, you are free to customize deployments further.
...
* *IBM WebSphere Liberty and Open Liberty on AKS:* You will need to use the IBM WebSphere Liberty and Open Liberty on AKS on Azure marketplace. See the official
https://learn.microsoft.com/en-us/azure/aks/howto-deploy-java-liberty-app?tabs=in-bash#create-a-liberty-on-aks-deployment-using-the-portal[Create a Liberty on AKS deployment using the portal^] documentation for more information. Or, you can go directly to https://aka.ms/liberty-aks[the offer]. 
...
== Use IBM Liberty on AKS tool to provision a resource group, container registry and Kubernetes cluster
...
To create the resource group in an efficient way, check out the https://aka.ms/liberty-aks[IBM Liberty on AKS] tool and click on the `Create` button to start the process. 

On the `Basics` pane. Under the `Resource group`, select `Create new` and type in **guideGroup** as the name of the resource group.

image::AKS_tool_create_resource_group.png[AKS tool create resource group,align="center"]

An Azure location must be specified. The metadata for your resources are stored at this specified Azure location. If resources are created later without specifying a location, these new resources will run in the same region that you specified for creating a resource group. Click on the `Region` drop down menu and select the desired location.
...
=== Creating a container registry and an AKS cluster

Once finish setting up the resource group, click on `Next` and move on to the `AKS` pane. 

To deploy your microservices, you need to create an Azure Container Registry in the same location where your services are deployed and link it to a resource group if using command line. However, when leveraging the `IBM Liberty on AKS`, this linkage will be automatically setup during the process. The created registry will manage container instances that will be deployed to a Kubernetes cluster. 

You will also need to create a AKS Cluster which helps to host the containerized applications which managed by Kubernetes on the Azure.

In the `AKS` pane, you can optionally select an existing AKS cluster and container registry instance. You can also adjust the settings for the size and number of the virtual machines in the AKS node pool.

From this guide's perspective, you can keep all the defaults on the pane as shown below, and the tool will create a new AKS cluster and a container registry instance. 

image::Create_AKS_and_ACR.png[Create AKS and ACR,align="center"]

=== Finishing the creations

For pane `Load balancing` and `Operator and application`, you can keep the default option in this guide. For more information about the configuration in these two panes, please visit the documentation of https://learn.microsoft.com/en-us/azure/aks/howto-deploy-java-liberty-app?tabs=in-bash#create-a-liberty-on-aks-deployment-using-the-portal[Create a Liberty on AKS deployment using the portal]

Select **Review + create** button to validate your setup. Once you complete the review and the validation process, select the **Create** button to initiate the deployment process, which may take up to 20 minutes to complete.

// =================================================================================================
// Uploading images to a container registry
// =================================================================================================

Upon completion of the deployment process, navigate to the resource group overview page in the Azure Portal. Review the created resource group and search for resources of the type `Container registry`. Locate the container registry that was created during the deployment. The name of the Container Registry will be used in `[registry-name]`.
...
=== Merge the cluster credentials into the current Kubernetes configuration

// An AKS cluster requires a service principal, which is an identity that is used to represent a resource in Azure that can be assigned roles and permissions to interact with other resources and the Azure API. The `az aks create` command automatically generates a service principal to use with your newly created cluster.Optionally, you can https://docs.microsoft.com/en-us/azure/aks kubernetes-service-principal#manually-create-a-service-principal[manually create a service principal^]yourself and create a cluster with this new service principal.However, to run this command, your Azure account must have permission access to create service principals. 

// service principal is being replace by a Managed identity: use the command to find service principle and identity `az aks show --name cluster-name --resource-group resource-group name`
// =================================================================================================

To locate the created Kubernetes cluster, go to the resource group overview page in the Azure Portal. Look for resources of type `Kubernetes service` and find the cluster with a name in the format `clusterXXXXXX`. Replacing `[cluster-name]` with the name of the cluster.

image::Locate_created_cluster.png[Locate created cluster,align="center"]
...
[role="no_copy"]
----
Merged created cluster" as current context in /Users/.kube/config
----
