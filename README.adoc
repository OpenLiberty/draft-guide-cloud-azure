...
Select **Review + create** pane to validate your setup. Once you complete the review and the validation process, click on the **Create** button to initiate the deployment process, which may take up to 20 minutes to complete.
...
Upon completion of the deployment process, navigate to the resource group overview page in the Azure Portal. Review the created resource group and search for resources of the type `Container registry`. Locate the container registry that was created during the deployment. The container registry name should be substituted with `[registry-name]`. For example, use "acrc75e71" as seen under the `Name` column in the output.
...
To push Docker images to your registry, you must log in to your Azure Container Registry by using the Azure CLI. Replace the [registry-name] with the name of the container registry.
...
Replace `[registry-server]` with the login server name of your container registry.
