...
* *IBM WebSphere Liberty and Open Liberty on AKS:* You will need to use the IBM WebSphere Liberty and Open Liberty on AKS plan on Azure marketplace. See the official
...
== Provision deployment foundation with Liberty on AKS 
...
To create the resource group in an efficient way, check out the https://aka.ms/liberty-aks[IBM WebSphere Liberty and Open Liberty on AKS] plan and click on the `Create` button to start the process. 

On the `Basics` pane. Under the `Resource group`, click on the `Create new` link and type in `guideGroup` as the name of the resource group. Please note, the offer also supports to use an existing resource group while the group is required to be empty. 
...
To deploy your microservices, you need to create an Azure Container Registry in the same location where your services are deployed and link it to a resource group if using command line. However, when leveraging the `IBM WebSphere Liberty and Open Liberty on AKS` plan, this linkage will be automatically setup during the process. The created registry will manage container instances that will be deployed to a Kubernetes cluster. 

Additionally, you will need to create an AKS Cluster, which is used to host containerized applications managed by Kubernetes on Azure.
...
aks-agentpool-39342667-vmss000000   Ready    agent   20h   v1.28.10
----
